 ...
 */
var QYLIN_REPORT = (function(global) {
    if (global.QYLIN_REPORT) return global.QYLIN_REPORT;
                ...
                if (!T.isEmpty(value)) {
                    if (T.isOBJ(value)) {
                        try {
                            ...
                            value = "[QYLIN_REPORT detect value stringify error] " + err.toString();
                        }
                    }
    ...
    var report = global.QYLIN_REPORT = {
        ...
        reportOfflineLog : function (){
            if (!window.indexedDB){
                QYLIN_REPORT.info("unsupport offlineLog");
                return ;
            }
            Offline_DB.ready(function (err , DB){
                if(!DB){
    ...
    typeof console !== "undefined" && console.error && setTimeout(function() {
        var err = ((location.hash || "").match(/([#&])QYLIN_ERROR=([^&$]+)/) || [])[2];
        err && console.error("QYLIN_ERROR", decodeURIComponent(err).replace(/(:\d+:\d+)\s*/g, "$1\n"));
    }, 0);
